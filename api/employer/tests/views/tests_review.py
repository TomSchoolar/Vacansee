...
class getReviewTests(TestCase):
    ...
    def test_validRequestFullData(self):
        # make request
        response = self.client.get(f'/v1/e/reviews/{ self.vacancyId }/', **{'HTTP_AUTHORIZATION': f'Bearer: { self.jwt }'})
        data = response.data
    ...
    def test_otherUsersVacancy(self):
        ...
        response = self.client.get(f'/v1/e/reviews/{ vacancyId }/', **{'HTTP_AUTHORIZATION': f'Bearer: { self.jwt }'})
    ...
    def test_expiredJwt(self):
        ...
        response = self.client.get(f'/v1/e/reviews/{ self.vacancyId }/', **{'HTTP_AUTHORIZATION': f'Bearer: { jwt }'})
...
class putReviewValidTests(TransactionTestCase):
    ...
    def test_validAccept(self):
        getPreResponse = self.client.get(f'/v1/e/reviews/{ self.vacancyId }/', **{'HTTP_AUTHORIZATION': f'Bearer: { self.jwt }'})
        putResponse = self.client.put(
            f'/v1/e/reviews/{ self.vacancyId }/updatestatus/{ self.applicationId }/',
            data={ 'setStatus': 'accept' },
            content_type='application/json',
            **{'HTTP_AUTHORIZATION': f'Bearer: { self.jwt }'}
        )
        getPostResponse = self.client.get(f'/v1/e/reviews/{ self.vacancyId }/', **{'HTTP_AUTHORIZATION': f'Bearer: { self.jwt }'})
...
class putReviewInvalidTests(TestCase):
    ...
    def test_invalidApplication(self):
        ...
        response = self.client.put(
            f'/v1/e/reviews/{ self.vacancyId }/updatestatus/{ applicationId }/',
            data={ 'setStatus': 'reject' },
            content_type='application/json',
            **{'HTTP_AUTHORIZATION': f'Bearer: { self.jwt }'}
        )
    ...
    def test_otherUsersVacancy(self):
        ...
        response = self.client.put(
            f'/v1/e/reviews/{ vacancyId }/updatestatus/{ self.applicationId }/',
            data={ 'setStatus': 'reject' },
            content_type='application/json',
            **{'HTTP_AUTHORIZATION': f'Bearer: { self.jwt }'}
        )
